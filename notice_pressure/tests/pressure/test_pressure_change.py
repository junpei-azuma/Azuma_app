...
@pytest.mark.parametrize("current_datetime, past_datetime",
...
def test_時間差分計算_戻り値が偽(current_datetime: datetime, past_datetime: datetime):
    ...
    assert not PressureChange.validate_datetime_difference(current_pressure, pressure_ago)
...
def test_インスタンス生成正常系():
    ...
    pressure_change: PressureChange = PressureChange(current_pressure, pressure_3hour_ago)
